...
async function load() {
  ...
  try {
    ...
    items.value = (data.items ?? []) as ApelacionItem[]
  } catch (err) {
...
watch([selectedLocal, fechaDesde, fechaHasta], () => {
  if (selectedLocal.value && fechaDesde.value && fechaHasta.value) load()
})
watch(selectedSede, () => {
...
<template>
  <div class="mis-apelaciones-view">
    <Card class="mb-3">
      <template #title>Estado de mis apelaciones</template>
      <template #content>
        <p class="text-color-secondary mt-0 mb-3">
          Consulta el estado de las apelaciones de tu sede: pendientes de apelar, apeladas, reembolsadas o descuento en nómina.
        </p>
        <div class="flex flex-wrap gap-3 align-items-end mb-2">
          ...
          <div class="flex flex-column gap-2">
            <label for="mis-apel-desde">Desde</label>
            <DatePicker
              id="mis-apel-desde"
          ...
          <div class="flex flex-column gap-2">
            <label for="mis-apel-hasta">Hasta</label>
            <DatePicker
              id="mis-apel-hasta"
    ...
    <Card>
      ...
      <template #content>
        ...
        >
          ...
          <template #empty>
            <span class="text-color-secondary">No hay apelaciones para esta sede en el rango de fechas.</span>
          </template>
          <template #loading>
